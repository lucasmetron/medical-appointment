...
export const IntentSchema = z.object({
  intent: z
    .enum(["schedule", "cancel", "returnDoctors", "unknown"])
...
export const getSystemPrompt = (professionals: any[]) => {
  return JSON.stringify({
    ...
    rules: {
      returnDoctors: {
        description:
          "User is asking for the list of available doctors/professionals",
        keywords: [
          "doctors",
          "professionals",
          "who can I see",
    ...
    extraction_instructions: {
      professionalId:
        "Match the professional name mentioned in the question to the ID from the professionals list. Use fuzzy matching only when it is clearly the same professional. If no professional matches, do not set professionalId.",
      professionalName:
    ...
    examples: [
      ...
      {
        input: "Can I see the list of available doctors?",
        output: {
          intent: "returnDoctors",
          professionals:
            "Os médicos disponíveis são: Dr. Alicio da Silva (Cardiologista), Dr. Ana Pereira (Dermatologista), Dr. Carlos Souza (Pediatra)",
        },
      },
      {
